...
class Test_003(unittest.TestCase):
    ...
    def test_003(self):
        ...
        finalizar = Selenium.get_elements(self, "Finalizar_compra")
        texto_finalizar = finalizar.text
        finalizar_esperado = "Finalizar compra"
        self.assertEqual(texto_finalizar, finalizar_esperado, f"El botón ({texto_finalizar}) no se encuentra ({finalizar_esperado}).")
        print("Se visualiza correctamente el botón 'Finalizar compra'.")
        time.sleep(5)
        
        self.driver.save_screenshot('../Data/Evidencia/captura3.png')
        time.sleep(5)
